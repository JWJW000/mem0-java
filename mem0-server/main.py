...
TOKEN = os.environ.get("TOKEN", "123123")
...
async def auth_middleware(request: Request, call_next):
    ...
    if not token or not token.startswith("Token " + TOKEN):
...
def get_all_memories(
    ...
    try:
        ...
        return MEMORY_INSTANCE.get_all(**params ,limit=page_size)
    except Exception as e:
...
def update_memory(memory_id: str, updated_memory: Dict[str, Any]):
    ...
    try:
        return MEMORY_INSTANCE.update(memory_id=memory_id, data=updated_memory['message'])
    except Exception as e:
